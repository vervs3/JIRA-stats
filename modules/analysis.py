...
def prepare_chart_data(df, data_source='jira', use_filter=True, filter_id=None, jql_query=None,
                       ...
                       implementation_issues=None, filtered_issues=None):
    """
    ...
    try:
        ...
        if data_source == 'jira':
            ...
            if date_from or date_to:
                ...
                # Get issues with worklog for the specified period, but ONLY those that are in implementation_issues
                analysis_state['status_message'] = f'Filtering implementation issues by worklog date: {date_query}'

                # Extract all implementation issue keys
                implementation_issue_keys = [issue.get('key') for issue in implementation_issues if issue.get('key')]
                logger.info(f"Found {len(implementation_issue_keys)} implementation issue keys to filter")

                if not implementation_issue_keys:
                    # If no implementation issues, return empty result
                    analysis_state['status_message'] = "No implementation issues found to filter by date."
                    analysis_state['is_running'] = False
                    return

                # Create batches of issue keys for filtering (Jira has query length limitations)
                filtered_issues = []
                total_issues_count = 0
                batch_size = 50  # Use smaller batches for combined queries

                # Process issues in batches to avoid query length limitations
                for i in range(0, len(implementation_issue_keys), batch_size):
                    batch_keys = implementation_issue_keys[i:i + batch_size]
                    # Create JQL with issue keys AND date filter
                    batch_query = f"key in ({','.join(batch_keys)}) AND ({date_query})"
                    ...
                    progress_pct = int((i + batch_size) / len(implementation_issue_keys) * 100)
                    progress_pct = min(progress_pct, 100)
                    analysis_state['progress'] = 25 + int(progress_pct * 0.15)  # Scale to 25-40% progress
                    analysis_state[
                        'status_message'] = f'Processed {i + batch_size}/{len(implementation_issue_keys)} issues, found {total_issues_count} with worklogs'

                logger.info(
                    f"Filtered from {len(implementation_issue_keys)} implementation issues to {len(filtered_issues)} issues with worklogs")
...
def map_components_to_projects(est_issues, implementation_issues):
    ...
    special_mappings = {
        # UNIGUI мачится на NBSSPORTAL
        'UNIGUI': ['NBSSPORTAL'],

        # PRAIM_INV, PRAIM мачится на CHM (не CHR)
        'PRAIM_INV': ['CHM'],
        'PRAIM': ['CHM'],

        # NBSS или NUS на NBSSPORTAL или NUS
        'NBSS': ['NBSSPORTAL', 'NUS'],
        'NUS': ['NBSSPORTAL', 'NUS'],

        # UDB_INV, UDB могут мачится на UDB или ATS,SSO
        'UDB_INV': ['UDB', 'ATS', 'SSO'],
        'UDB': ['UDB', 'ATS', 'SSO']
    }
    ...
    for component in components:
        # Check if this component has a special mapping rule
        if component in special_mappings:
            # Use the predefined mapping and ensure projects exist
        ...
        # Normal substring matching for other components
        matched_projects = []
